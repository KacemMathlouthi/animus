...
- Subclass VoiceoverScene (\`from manim_voiceover import VoiceoverScene\`), not Scene. As the FIRST line of construct, set the speech service exactly: \`self.set_speech_service(ElevenLabsService(voice_name="Rachel", model_id="eleven_multilingual_v2"))\` (\`from manim_voiceover.services.elevenlabs import ElevenLabsService\`). The API key is already in the environment — NEVER put a key in the code.
