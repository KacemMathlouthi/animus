 ...
 * `self.voiceover(text=...)` calls in the scene source.
 *
 ...
 * caches unchanged lines and does not re-synthesize them). An exact accounting
 * would diff manim-voiceover's on-disk cache; that is a deliberate fast-follow.
 * The parser is intentionally forgiving — if it recognizes nothing, it returns
 * 0 (we would rather under-charge than over-charge on an unparseable scene). */

/** Matches `voiceover(text=<string literal>` allowing an optional f/r/b prefix
 * and single, double, or triple quotes. The opening quote is captured so the
 * matching close can be found. */
